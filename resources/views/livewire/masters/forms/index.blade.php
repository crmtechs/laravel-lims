                        ...
                        <table class="table table-hover align-middle mb-0">
                                    ...
                                    <th>
                                        <div class="dropdown d-inline-block">
                                            ...
                                            <ul class="dropdown-menu shadow text-sm">
                                                <li><a class="dropdown-item" href="#" wire:click.prevent="selectPage">{{ __('global.select_this_page') }} ({{ $forms->count() }})</a></li>
                                                <li><a class="dropdown-item" href="#" wire:click.prevent="selectAll">{{ __('global.select_all') }} ({{ $forms->total() }})</a></li>
                                                <li><hr class="dropdown-divider"></li>
                                                <li><a class="dropdown-item" href="#" wire:click.prevent="deselectAll">{{ __('global.deselect_all') }}</a></li>
                                            </ul>
                                        </div>
                                    </th>
                                    <th class="w-20">{{ __('forms_master.document_name') }}</th>
                                    <th class="w-20">{{ __('forms_master.document_title') }}</th>
                                    <th class="w-15">{{ __('forms_master.status') }}</th>
                                    <th class="w-15">{{ __('forms_master.publish_date') }}</th>
                                    <th class="w-15">{{ __('global.assigned_to') }}</th>
                                    <th class="w-15">{{ __('global.created_at') }}</th>
                            ...
                            <tbody>
                                @forelse($forms as $form)
                                    ...
                                    @endphp
                                    <tr wire:key="form-{{ $form->uuid }}">
                                        <td>
                                            <input type="checkbox" wire:key="checkbox-{{ $form->uuid }}" wire:model.live="selected" value="{{ $form->uuid }}" class="form-check-input cursor-pointer">
                                        </td>
                                        <td>
